...
export default function AmbulancePanel() {
  ...
  const unitStatus = myActiveEmergency ? myActiveEmergency.status : 'available';

  // Build map data for the navigation map
  ...
  const mapHospitals = myActiveEmergency?.hospital ? [{
    ...
    available_beds: 1, // Just to make it green
  ...
  return (
    <div className="space-y-4 animate-slide-in-up">
      ...
      {/* ── Navigation Map (when active emergency) ── */}
      {myActiveEmergency && (
      ...
      {/* ── Emergency Dispatch Form (no active emergency, and NOT read-only) ── */}
